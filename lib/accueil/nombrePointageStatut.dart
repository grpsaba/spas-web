...
class _NbAgentStatusState extends State<NbPointageStatus> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: PointingSiteService().allBySupervisor(supervisor: widget.supervisor),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            ...
            var collection =
                docs?.map((e) => PointingSite.fromJson(e)).where((p) => p.date.day==today.day && p.date.year==today.year && p.date.month==today.month).toList();
